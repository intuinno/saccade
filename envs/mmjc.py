...
class MMJCNavEgo:
    """Navigation wrapper with egocentric observations, sparse reward, and curriculum.

    Like MMJCNavCont but replaces allocentric current_position/target_position/heading
    (6D) with a single ego_nav vector [distance, angle] (2D), where distance is
    Euclidean to goal and angle is relative bearing in agent's frame (radians, [-pi, pi]).
    """
    ...
    def _build_obs(self, base_obs, info, is_first, is_last, is_terminal):
        agent_pos = info["agent_pos"]
        agent_dir = info["agent_dir"]
        delta = self._goal_pos - agent_pos
        distance = np.linalg.norm(delta)
        angle = np.arctan2(delta[1], delta[0]) - np.arctan2(agent_dir[1], agent_dir[0])
        angle = (angle + np.pi) % (2 * np.pi) - np.pi  # wrap to [-pi, pi]
        return {
            "image": base_obs["image"],
            "ego_nav": np.array([distance, angle], dtype=np.float32),
            "proprioceptors": base_obs["sensors"],
            "is_first": is_first,
            "is_last": is_last,
            "is_terminal": is_terminal,
    ...
    def _bfs_distances(self, maze_map, agent_pos):
        """BFS from agent cell, return dict mapping (gx, gy) -> path distance."""
        rows, cols = maze_map.shape
        ac = int(agent_pos[0]) + 1
        ar = (rows - 2) - int(agent_pos[1])
        dist = {}
        queue = deque([(ar, ac, 0)])
        visited = set()
        visited.add((ar, ac))
...
class MMJCHierNavEgo:
    """Hierarchical navigation wrapper with egocentric observations.

    Like MMJCHierNav but replaces allocentric current_position/target_position/heading
    (6D) with a single ego_nav vector [distance, angle] (2D), where distance is
    Euclidean to goal and angle is relative bearing in agent's frame (radians, [-pi, pi]).
    """
